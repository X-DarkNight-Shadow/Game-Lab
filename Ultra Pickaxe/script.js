...
var UpgradePrice = {
    ...
    Luck:        [0.5, 1, 2, 4, 7, 11, 16, 22, 30, 40, 55, 75, 100, 140, 200],
    Efficiency:  [0.5, 0.8, 1.5, 3, 5, 8, 12, 18, 26, 36, 50, 70, 95, 130, 150],
    Quality:     [0.05, 0.15, 0.4, 1, 2, 4, 7, 11, 16, 22, 30, 42, 58, 80, 100],
...
var ShopItemPrice = {
    ...
    C4: 100,
    Artillery_Shell: 750,
    Anti_Tank_Guided_Missile: 1500,
    Heavy_Tank_Cannon_Round: 5000,
    Worker: 10
};
...
function UpdateMaterialUI(){
    ...
    for (const Material in MaterialValue){
        ...
        TotalEarningsValue += MaterialValue[Material] * MaterialPrice[Material] * Upgrade.Quality;
    }

    document.getElementById("TotalEarnings").textContent = "(+" + TotalEarningsValue + " €)";
}
...
function UpdateUpgradeUI(){
    for (const upgrade in UpgradePriceLevel){
        ...
        let cost = UpgradePrice[upgrade][level];

        if (cost === undefined){
            const prices = UpgradePrice[upgrade];
            const previousCost = prices[prices.length - 1];

            if (previousCost === false){
                cost = false;
            } else {
                cost = previousCost * 2;
                prices.push(cost);
...
function BuyUpgrade(upgrade){
    ...
    if (cost === undefined){
        ...
        if (previousCost === false || previousCost === undefined){
            return;
        }
